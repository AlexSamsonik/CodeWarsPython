...
def add_letter_s_to_time_period_name(count_period: str, time_period_name: str) -> str:
    """Add letter 's' to time period name if more than one.

    :param count_period: count period.
    :param time_period_name: time period name e.g. year, day, hour, minute, second
    """
    if count_period == 1:
        return f"{count_period} {time_period_name}"
    return f"{count_period} {time_period_name}s"
...
def format_duration(seconds):
    ...
    time_period_list = [[365 * 24 * 60 * 60, "year"],
                        [24 * 60 * 60, "day"],
                        [60 * 60, "hour"],
                        [60, "minute"],
                        [1, "second"]]
    ...
    for time_period in time_period_list:
        time_period_seconds, time_period_name = time_period
        if seconds >= time_period_seconds:
            count_period = seconds // time_period_seconds
            human_format.append(add_letter_s_to_time_period_name(count_period, time_period_name))
            seconds = seconds - count_period * time_period_seconds

    return " and ".join(", ".join(human_format).rsplit(", ", 1))
